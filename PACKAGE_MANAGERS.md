...
Generator 10 pins the published runtime line exactly, including `white-label-view@7.0.0`, and the committed lockfile resolves those dependencies from the npm registry. The security workflow also substitutes a fixed packed View 7 source revision in coordinated integration jobs; that is an additional cross-repository compatibility check, not a workaround for an unpublished View release. Keep the release lockfile registry-based rather than replacing runtime dependencies with Git or local package references.
